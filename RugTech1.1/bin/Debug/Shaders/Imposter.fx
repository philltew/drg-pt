...
{
	float4 Color = (1 - imposter.Sample(ImposterSampler, input.texCoord)) * float4(brightPassThreshold, brightPassThreshold, brightPassThreshold, 1);
...
float4 Imposter_PS_BrightPass( PS_FINAL_IN input ) : SV_Target
{
	...
	float4 value = imposter.Sample(ImposterSampler, input.texCoord); 

	// Determine the luminance of this particular pixel
	float luminance = dot(value.rgb, luminanceVector);

		...
		public bool InitializeRandomParticlePosition(int particleIdx, int Height, int Width)
		{
			...
			{ // only execute if it's the first and only particle
				...
				do
				{
					...
					for (jj = 0; jj < i; ++jj)
					{
						//double xdif = GetXParticlePosition(i) - GetXParticlePosition(jj);
						//double ydif = GetYParticlePosition(i) - GetYParticlePosition(jj);
						...
						//ijjSeparation = Math.Sqrt((xdif * xdif) + (ydif * ydif));
  ...
  if((GetForceFieldObject(0)->getForceFieldType())=="SoftSphereForceField"){
	GetForceFieldObject(0)->updateEnergyTerms(this);      
		...
		public void CalculateKineticEnergiesAndTemperature()
		{
			...
			for (int i = 0; i < GetNumberOfParticles(); ++i)
			{
				double vx = GetXParticleVelocity(i);
				double vy = GetYParticleVelocity(i); 

				KE = 0.5 * GetParticleMass(i) * (Math.Pow(GetXParticleVelocity(i), 2.0) + Math.Pow(GetYParticleVelocity(i), 2.0));
				//KE = 0.5 * GetParticleMass(i) * ((vx * vx) + (vy * vy));
			...
			for (int i = 0; i < GetNumberOfParticles(); ++i) { sum += Math.Pow((pParticleVector[i].getKineticEnergy() - AverageKineticEnergy), 2.0); }
		...
		public void UpdateInterParticleSeparations()
		{
			...
			for (int i = 0; i < NumberOfParticles; ++i)
			{
				for (int j = (i + 1); j < NumberOfParticles; ++j)
				{
					...
					double xdif = pParticleVector[i].getpx() - pParticleVector[j].getpx();
					double ydif = pParticleVector[i].getpy() - pParticleVector[j].getpy();
					...
					//ijSeparation = Math.Sqrt((xdif * xdif) + (ydif * ydif));

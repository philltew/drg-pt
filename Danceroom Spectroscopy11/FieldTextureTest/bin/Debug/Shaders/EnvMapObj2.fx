...
cbuffer cbConstant
{
    float3 vLightPos = float3(50,100,30); //float3(-0.577,0.577,-0.577);
};
...
/* 
	...
	*/ 
	...
	//float4 normalMap = texture2D(tex2, gl_TexCoord[0].xy); 
	float3 normal = input.Norm; // normalize(vec3((normalmap.x-0.5)*2.0, (normalmap.z-0.5)*2.0, normalmap.y-0.5)*-2.0)); 

	float3 light_pos = vLightPos; // float4(vLightPos, 1);
	//light_pos = mul(light_pos, World);    
    //light_pos = mul(light_pos, View);
    //light_pos = mul(light_pos, Projection);

	float NdotL = max(0.0, dot(light_pos, normal)); 
	float3 diffuse_color = float3(NdotL * 1.0, NdotL * 1.0, NdotL * 1.0); 

	float3 diffuse_map_vec = normal; // obj2world * normal;
	diffuse_map_vec.y *= -1.0; 
	diffuse_color += texDiffuseMap.Sample(sampDiffuseMap, diffuse_map_vec).xyz * 0.5; 

	float3 H = normalize(normalize(input.Pos2 * -1.0) + normalize(light_pos));
	float spec = min(1.0, max(0.0, pow(abs(dot(normal,H)), 10.0) * 1.0 * NdotL)); 
	float3 spec_color = float3(spec, spec, spec); 

	float3 spec_map_vec = reflect(input.Pos2, normal); 
	spec_map_vec = spec_map_vec; // obj2world * spec_map_vec;
	spec_map_vec.y *= -1.0; 
	spec_color += texSpecularMap.Sample(sampSpecularMap, spec_map_vec).xyz * 0.5; 

	float4 colormap = texColorMap.Sample(sampColorMap, input.Tex); 

	float3 diffuse_final = diffuse_color * colormap.xyz;
	float3 spec_final = spec_color * colormap.a; 

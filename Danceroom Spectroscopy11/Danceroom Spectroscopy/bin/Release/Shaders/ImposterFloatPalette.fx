...
float4 Imposter_PS( PS_FINAL_IN input ) : SV_Target
{
	...
	float4 palColor = palette.Sample(ImposterSampler, float2(paletteHue, col)); 

	return palColor * float4(brightPassThreshold, brightPassThreshold, brightPassThreshold, 1);
}
...
float4 Imposter_Invert_PS( PS_FINAL_IN input ) : SV_Target
{
	...
	float4 palColor = palette.Sample(ImposterSampler, float2(paletteHue, value));

	float4 Color = (1 - palColor) * float4(brightPassThreshold, brightPassThreshold, brightPassThreshold, 1);
...
float4 Imposter_PS_BrightPass( PS_FINAL_IN input ) : SV_Target
{
	...
	float4 value4 = palette.Sample(ImposterSampler, float2(paletteHue, value));

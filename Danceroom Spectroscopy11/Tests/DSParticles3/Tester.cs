...
namespace DSParticles3
{
	public class Tester
	{
		...
		private void SetupForTesting(int Nparticles)
		{
			...
			ParticleStaticObjects.AtomPropertiesDefinition.AddParticleDefinition("A", 2, unchecked((int)0xc00A32FF), 0, 50); // neon blue
			ParticleStaticObjects.AtomPropertiesDefinition.AddParticleDefinition("B", 4, unchecked((int)0xc0FF0A32), 1, 49); // red
			ParticleStaticObjects.AtomPropertiesDefinition.AddParticleDefinition("C", 6, unchecked((int)0xc0320AFF), 2, 45); // purple
			ParticleStaticObjects.AtomPropertiesDefinition.AddParticleDefinition("D", 9.2, unchecked((int)0xc026D8FF), 3, 43); // sky blue // 0xc032FF0A, 3,  43); // green 
			ParticleStaticObjects.AtomPropertiesDefinition.AddParticleDefinition("E", 12.6, unchecked((int)0xc0FF320A), 4, 38); // orange
		...
		{			
			...
			for (int i = 0; i < iterations; i++)
			{
				...
				{
					Ensemble1.VelocityVerletPropagation(Field);
				}
			}

...
namespace DSParticles3
{
	public class ParticleEnsemble
	{
		...
		*/ 
		...
		/// <summary>
		/// Get the paritcle at the given index 
		/// </summary>
		/// <param name="i"></param>
		/// <returns></returns>
		...
		public void VelocityVerletPropagation(ExternalField pExternalField)
		{
			...
			else
			{
				...
				for (i = 0; i < NumberOfParticles; ++i)
				{
					...
					GetParticle(i).GridSector.GetSector(GetParticle(i).Position);
				}
...
// THIS ONE !UsePOW
		...
		public void UpdateInterParticleSeparations()
		{
			...
			for (int i = 0; i < NumberOfParticles; ++i)
			{
				...
				for (int j = (i + 1); j < NumberOfParticles; ++j)
				{
					Particle particlej = GetParticle(j);
		...
		public void FFTVelocityAutoCorrelationFunction()
		{
			...
			for (int i = 0; i < Particles.Count; ++i)
			{
				for (int jj = 0; jj < 1024; ++jj)
				{
					...
					AverageVelocityAutoCorrelationFunction[jj] += avgfac * (Particles[i].VelocityAutoCorrelationFunction[0]) * (Particles[i].VelocityAutoCorrelationFunction[jj]);
				}
			}

...
Part_PS_IN Part_VS(Part_VS_IN input)
{
	...
	float scale = input.instanceCol.w * partScale; 

	float4 inputPos = float4(input.pos.x * scale, input.pos.y * scale, 0, 0);

	output.pos = inputPos + float4(input.instancePos.x, input.instancePos.y, input.instancePos.z, 1); // mul(float4(input.instancePos[0], input.instancePos[1], input.instancePos[2], 1.0f), worldViewProj);
	//output.pos = input.pos + mul(float4(input.instancePos[0], input.instancePos[1], input.instancePos[2], 1.0f), worldViewProj);
	output.texCoord = input.texCoord;		 
	output.col = float4(input.instanceCol.x, input.instanceCol.y, input.instanceCol.z, 1) * ampScale; // (input.instanceCol * ampScale) * saturate((maxDistance - output.pos.z) * (scaleDistance));
	//output.col.a = 1f; 

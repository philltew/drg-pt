...
{
	public class ImposterFloatPaletteEffect : BasicEffectBase
	{
		#region Private Members

		private string m_ShaderLocation = @"~/Shaders/ImposterFloatPalette.fx";
		...
		private EffectResourceVariable m_PaletteTextureResource;
		private EffectScalarVariable m_PaletteHue;

		private Texture2D m_PaletteTexture;
		private ShaderResourceView m_PaletteResourceView; 
		...
		public override void UnloadResources()
		{
			if (m_Disposed == false)
			{
				...
				m_PaletteResourceView.Dispose();
				m_PaletteTexture.Dispose();
				m_Vertices.Dispose();
				m_Layout.Dispose();
				m_Effect.Dispose();
			}
...
//			GameEnvironment.Device.OutputMerger.DepthStencilState = GameEnvironment.DepthState;
		...
		{
			RenderImposter(overlay, imposterTexture, 1f, 0.5f); 
		}

		public void RenderImposter(ImposterOverlayType overlay, VertexBufferBinding bindings, ShaderResourceView imposterTexture)
		{
			RenderImposter(overlay, bindings, imposterTexture, 1f, 0.5f);
		}

		public void RenderImposter(ImposterOverlayType overlay, ShaderResourceView imposterTexture, float brightPassThreshold, float color)
		{
			if (m_Disposed == true)
			{
				return;
		...
		public void RenderImposter(ImposterOverlayType overlay, VertexBufferBinding bindings, ShaderResourceView imposterTexture, float brightPassThreshold, float color)
		{
			if (m_Disposed == true)
			{
				return;
